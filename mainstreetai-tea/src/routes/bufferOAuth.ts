...
import {
  ...
  completeBufferOauthAndSave,
  createBufferOauthState,
} from "../integrations/bufferOauth";
...
router.get("/start", verifyAuth, async (req, res, next) => {
  ...
  if (typeof rawBrandId !== "string" || rawBrandId.trim() === "") {
    return res.status(400).json({
      error:
        "Missing brandId query parameter. Example: /api/integrations/buffer/start?brandId=main-street-nutrition",
    });
  }
...
router.get("/callback", async (req, res, next) => {
  ...
  try {
    const result = await completeBufferOauthAndSave({
      code,
      stateToken: state,
    });

    const redirectPath = `/admin/integrations/buffer?brandId=${encodeURIComponent(
      result.brandId,
    )}&status=connected`;
