...
import {
  listTownSeasonRefreshTargets,
  refreshTownSeasonNotesTemplatesForTown,
} from "../services/townSeasonService";
...
async function processTownSeasons(): Promise<{
  due: number;
  processed: number;
  failed: number;
  updatedRows: number;
}> {
  const targets = await listTownSeasonRefreshTargets(24);
  let processed = 0;
  let failed = 0;
  let updatedRows = 0;
  for (const target of targets) {
    try {
      const result = await refreshTownSeasonNotesTemplatesForTown({
...
async function handler(reqSecret: string | undefined) {
  ...
  const result = await processTownSeasons();
  return {
    status: 200,
    body: {
      ok: true,
