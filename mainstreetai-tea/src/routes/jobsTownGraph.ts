...
import { listDueTownGraphTargets, recomputeTownGraphSuggestionsForTown } from "../services/townGraphService";
...
async function processTownGraph(): Promise<{
  due: number;
  processed: number;
  failed: number;
  updatedRows: number;
}> {
  const targets = await listDueTownGraphTargets(20);
  let processed = 0;
  let failed = 0;
  let updatedRows = 0;
  for (const target of targets) {
    try {
      const result = await recomputeTownGraphSuggestionsForTown({
...
async function handler(reqSecret: string | undefined) {
  ...
  const result = await processTownGraph();
  return {
    status: 200,
    body: {
      ok: true,
